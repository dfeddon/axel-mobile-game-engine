...
package net.pixelpracht.tmx 
{
	...
	public class TmxLayer
	{
		...
		public function TmxLayer(source:XML, parent:TmxMap,allowMultiTilesets:Boolean = false)
		{
			map = parent;
			name = source.@name;
			x = source.@x; 
			...
			if(data)
			{
				...
				if (!allowMultiTilesets && map.tilesetsCount > 1)
				{
					var gidowner:TmxTileSet;
					var oldowner:TmxTileSet;
					var gg:int;
					var l:int = tileGIDs.length;
					var l2:int;
					var c:int;
					for (i = 0 ; i < l ; i++)
					{
						l2 = tileGIDs[i].length;
						for (c = 0 ; c < l2 ; c++)
						{
							gg = tileGIDs[i][c];
							...
							if (gg > 0)
							{
								gidowner = map.getGidOwner(gg);
								
								if (gidowner != oldowner && oldowner != null)
								{
									//error!
									throw new Error("more than one tileset used in a layer!");
									//tileGIDs[i][c] = 0;
								}else
								{
									if (!oldowner)
									{
										_tilesetName = gidowner.name;
									}
									
									if (gidowner.firstGID > 1)
									{
										tileGIDs[i][c] = gg - gidowner.firstGID + 1;
									}
				...
				{
					_tilesetName = map.getTilesetByIndex(0).name;
				}
		...
		public static function base64ToArray(chunk:String, lineWidth:int, compressed:Boolean):Array
		{
			...
			while(data.position < data.length)
			{
				...
				for(var i:int = 0; i < lineWidth; i++)
				{
					...
					//trace("resultRow.push(" + resultRow[resultRow.length - 1].toString() + ")");
				}
				result.push(resultRow);
				//trace("result.push(" + result[result.length-1].toString() + ")");
			}
			return result;
		}
